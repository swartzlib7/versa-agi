...
- **File Monitor** — Reactive `inotifywait` daemon (parked). Designed for mid-minute event triggers — retained for future event-driven work. Skill distribution handled by Lifeline via status-based polling.
...
- **Install registration:** During setup (or update), you accept the BSL-1.1 license and **provide an email** (required) for release notes and to identify this install’s COA on VersaVoice. Setup derives a **COA call sign** from the email local-part (first 3 + last 3 characters) and prompts you to confirm or customize it; the COA’s external name becomes `Versa (callsign)` instead of a shared `Versa (COA)`. A minimal install event (version, platform, acceptance timestamp, email, call sign) may be sent to VersaVoice AI if a registration endpoint is configured. No agent task data or message content is included. If the endpoint is unavailable, the event is stored locally and retried silently when you open Mission Control (`agitop`). The install email is kept separate from your VersaVoice account email.
