...
# models.ini holds the user-owned model catalog (Edition 2.x) — treat it like a
# data file (the same way setup.ini is preserved): only seed the template on a
# fresh box where no models.ini exists yet. An existing deployed file is NEVER
# overwritten, so the [catalog_custom]/[providers_custom] user layer added via the
# CLI/dashboard survives. The setup.ini baseline ([catalog]/[providers]) is then
# regenerated by the Model Catalog step below (custom layer overlays it at read).
if [ -f "${SCRIPT_DIR}/models.ini" ]; then
...
" 2>/dev/null && ok "Views recreated with new columns" || warn "View recreation failed"
  ...
  if [ "${DRY_RUN}" = false ]; then
    ...
    if [ -f "${INI_FILE_PATH}" ]; then
      ...
      ok "Cloud model registry updated in setup.ini"
    fi
...
# ─── Unified Model Catalog Baseline + Sync (Edition 2.x) ──
# Runs for BOTH fresh installs and updates. models.ini has two origins:
#   • baseline  : [catalog]/[providers]       — regenerated here from setup.ini
#   • user layer: [catalog_custom]/[providers_custom] — added/edited via the
#                 CLI/dashboard, NEVER touched (the deployed file is preserved).
# Steps:
#   • `agictl model migrate` — rebuilds the baseline from setup.ini so model
#     additions/edits/removals in setup.ini propagate; the user layer is left
#     intact and overlays the baseline at read time (so live edits survive).
#   • `agictl model sync` — regenerates the derived label sections + the cloud/
#     third-party paths.env registries from the MERGED catalog.
# Both are idempotent and non-fatal. apply_system_permissions (below) re-asserts
# ownership/modes on the files they touch.
if [ "${DRY_RUN}" = false ] && command -v agictl >/dev/null 2>&1; then
  section "Model Catalog — Baseline & Sync"
...
if [ "${UPDATE_MODE}" = true ]; then
  # UPDATE MODE: setup.ini is READ-ONLY.
  # Values were read via ini_get at script start; nothing to write back.
  # Only legitimate INI write is cloud_models (handled in 5c-U via sed).
  if [ -f "${INI_FILE}" ]; then
    chmod 640 "${INI_FILE}"
    chown "${WATCHDOG_USER}:agi_agents" "${INI_FILE}"
    ok "Setup configuration verified at ${INI_FILE} (read-only during update)"
  fi
else
