# ─────────────────────────────────────────────────
# Versa AGi — Intel ARC SYCL Docker Image
#
# Builds llama.cpp with SYCL backend for Intel ARC GPUs.
# Based on upstream llama.cpp intel.Dockerfile, pinned to
# a verified driver/runtime combination.
#
# Build targets:
#   server — llama-server only (production)
#   full   — all binaries including llama-bench (testing)
#
# Usage:
#   docker build -t llama-sycl-server \
