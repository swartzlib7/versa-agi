...
`memory_management.md` says its awareness procedure must run before ending every **cycle**. There are no cycles in this mode. Run that procedure at the **end of a turn** when you did real work, and again before the Primary User turns the mode off.
...
The next Lifeline pulse will spawn you normally. If `status` says off, stop. Do not race a harness cycle.
