...
class CoaShippedMap(unittest.TestCase):
    ...
    def test_grok_dual_provider(self):
        self.assertEqual(keys_for_provider("xai"), ["grok-4.6"])
        self.assertIn("x-ai/grok-4.6", keys_for_provider("openrouter"))
        self.assertIn("grok-4.6", all_keys())
        self.assertIn("x-ai/grok-4.6", all_keys())

    def test_pairs_label_order(self):
        labels = [label for _, label in recommended_pairs("openrouter")]
        self.assertEqual(
            labels,
            [
                "Grok 4.6",
...
class StockLayerParity(unittest.TestCase):
    ...
    def test_provider_reported_reasoning_efforts(self):
        ...
        expected = {
            ...
            "z-ai/glm-5.2": ("high", ["none", "high", "xhigh"]),
        }
